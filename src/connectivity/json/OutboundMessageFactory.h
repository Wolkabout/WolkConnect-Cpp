...
public:
    ...
    static std::shared_ptr<OutboundMessage> makeFromSensorReadings(
      const std::string& deviceKey, std::vector<std::shared_ptr<SensorReading>> sensorReadings);
    static std::shared_ptr<OutboundMessage> makeFromAlarms(const std::string& deviceKey,
                                                           std::vector<std::shared_ptr<Alarm>> alarms);
    static std::shared_ptr<OutboundMessage> makeFromActuatorStatus(
      const std::string& deviceKey, std::vector<std::shared_ptr<ActuatorStatus>> actuatorStatuses);

    static std::shared_ptr<OutboundMessage> makeFromFirmwareUpdateResponse(
      const std::string& deviceKey, const FirmwareUpdateResponse& firmwareUpdateResponse);

    static std::shared_ptr<OutboundMessage> makeFromFilePacketRequest(const std::string& deviceKey,
    ...
    static std::shared_ptr<OutboundMessage> makeFromConfiguration(
...
private:
    ...
    static const constexpr char* CURRENT_CONFIGURATION_CHANNEL = "configurations/current/";
